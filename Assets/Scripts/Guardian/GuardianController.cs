    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        else
        {
            ...
            if (camemi != null)
            {
                ...
                // Aplicar daño siempre que el combate esté activo (no verificamos CanReceiveDamage
                // porque ese flag es para el combate jugador↔Camemi, no para ataques del aliado)
                ...
                {
                    // Si no hay combate activo en CombatManager, igual intentamos aplicar daño directamente
                    camemi.TakeDamage(damage);
                    Debug.Log($"¡Guardián aliado atacó a Camemi (fuera de CombatManager) infligiendo {damage} de daño!");
                }
            }
            else if (!other.CompareTag("Guardian") && !other.CompareTag("GuardianAlly") && !other.CompareTag("Player"))
            {
                // Enemigo genérico (no es guardián ni jugador)
                Debug.Log($"Guardián aliado golpeó a un enemigo ({other.name}).");
            }
        }
    }
